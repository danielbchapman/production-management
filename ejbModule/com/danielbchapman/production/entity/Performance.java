...
/**
 * A performance is an extension of an event that allows for some other
 * linked objects to be created. This adds considerable complexity to the 
 * calendar, but it does provide a way to interact with other parts of the
 * database.
 * 
...
public class Performance extends EventMapping
{
  ...
  @ManyToOne(cascade=CascadeType.ALL, optional=true, targetEntity=PerformanceAdvance.class)
	...
	public Performance()
  {
	...
	/**
   * The event sequence is a series of event related to a specific
   * performance. This includes all the events defaulted in by the 
   * performance schedule. This method is per-entity and allows 
   * quick and easy generation of the schedule so that the logic
   * can be in one place.
   * 
  ...
  public ArrayList<PerformanceEvent> getEventSequence()
  {
  	...
  	if(performanceSchedule == null)
  		return events;
  	if(crewCall)
  		events.add(getEvent("Crew Call", performanceSchedule.getCrewCall(), performanceSchedule.getCrewCall(), true, true, true));
  	
  	events.add(getEvent("AEA Call", performanceSchedule.getActorsEquityCall(), 30, true, true, true));
  	if(fightCall)
  		events.add(getEvent("Fight Call", performanceSchedule.getFightCall(), 15, true, true, true));
  	events.add(getEvent("House Open", performanceSchedule.getHouseCall(), performanceSchedule.getHouseCall(), true, true, true));
  	
  	if(talkback)
  	{
  		events.add(getEvent("Talkback", performanceSchedule.getPerformanceLength(), 15, false, true, true));
  		if(strike)
  			events.add(getEvent("Strike", performanceSchedule.getPerformanceLength() + 15, performanceSchedule.getStrikeLength(), false, true, true));
  	}
  	else
  		if(strike)
  			events.add(getEvent("Strike", performanceSchedule.getPerformanceLength(), performanceSchedule.getStrikeLength(), false, true, true));
  ...
  private final PerformanceEvent getEvent(String description, int offset, int duration, boolean eventBeforePerformance, boolean cast, boolean crew)
  {
  	...
  	return new PerformanceEvent(getDay(), description, startTime.getTime(), endTime.getTime(), cast, crew);
  }
  ...
  public class PerformanceEvent extends EventMapping
  {
		...
		public PerformanceEvent(Day day, String description, Date start, Date end, boolean cast, boolean crew)
		{
			setDay(day);
